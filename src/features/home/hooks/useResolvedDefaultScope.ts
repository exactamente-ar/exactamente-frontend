...
import {
  DEFAULT_FACULTY_SHORT,
  DEFAULT_UNIVERSITY_SHORT,
  DEFAULT_CAREER_SHORT,
} from '@/features/home/constants/filter';
import type { ResolvedDefaultScope } from '@/features/home/types/filter';

const matchShort = (a: string, b: string) => a.trim().toLowerCase() === b.trim().toLowerCase();
...
function writeCache(scope: ResolvedDefaultScope) {
  ...
  try {
    window.localStorage.setItem(CACHE_KEY, JSON.stringify({ ...scope, savedAt: Date.now() }));
  } catch {
    // localStorage puede fallar por cuota llena o modo privado. El cache es
    // solo una optimización, así que se sigue sin él.
  }
}
