...
const toFilterOptions = (items: { value: string; label: string }[]): FilterOption[] =>
  items.map(({ value, label }) => ({ id: value, label }));
      ...
      <form onSubmit={onSubmit} className='space-y-8'>
        ...
        <FormField label='Plan' required>
          <FilterCombobox
            ...
            placeholder={
              formData.careerId ? 'Seleccioná un plan' : 'Primero seleccioná una carrera'
            }
              ...
              {duplicateWarning?.hasSimilar && (
                <div className='mb-4 rounded-xl border border-yellow-500/40 bg-yellow-500/10 px-4 py-3 text-yellow-200'>
                  <p className='text-sm font-semibold mb-2'>
                    Ya existe un recurso similar en revisión o publicado. ¿Querés subirlo de todas
                    formas?
                  </p>
                  <button
                    type='button'
